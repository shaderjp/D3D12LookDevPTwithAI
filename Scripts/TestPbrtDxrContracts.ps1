...
Require-Pattern $pathTracing 'SampleDielectricContinuation\(.*surface\.material\.thinDielectric.*refract\(incidentDirection,\s*surface\.geometricNormal,\s*eta\).*throughputScale\s*=\s*\(eta\s*\*\s*eta\)' 'DXR dielectric sampling does not distinguish thin transmission from Snell refraction with the radiance Jacobian.'
...
Require-Pattern $pathTracing 'payload\.transmittance\s*\*=\s*ShadowMaterialTransmittance\(geometryIndex\).*IgnoreHit\(\)' 'DXR shadow any-hit does not accumulate dielectric transmittance.'
Require-Pattern $backend 'binding\.offsetScale\s*=\s*XMFLOAT4\(\s*source\.offset\.x\s*\+\s*material\.uvScaleOffset\.z.*source\.scale\.x\s*\*\s*material\.uvScaleOffset\.x' 'Path-tracing material bindings do not preserve the PBRT UV transform.'
